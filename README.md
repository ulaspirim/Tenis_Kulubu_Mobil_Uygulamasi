# Ulaş Tenis Kulübü - Mobil Uygulaması

## Proje Tanımı
Bu uygulama, çeşitli spor kulüp üyelerinin rezervasyon ve diğer uygulamalarının takibi, yönetimi ve sistemin kolaylaştırılması amacıyla kulübe üye kişiler ve kulüp yöneticileri için oluşturulmuştur.

[Proje Tanıtım Videosu](https://youtube.com/shorts/ivkp-HrG9Cw?feature=share)

## Proje Kategorisi
Sosyal Kulüp Ağı

## Referans Uygulama
Bu proje, bir çok rezervasyon ve kulüp yönetim sistemi için üretilmiş özgün bir çalışmadır ve daha önce oluşturulmuş çeşitli uygulamaların daha profesyonel, modern hali için oluşturulmuştur.

## Proje Ekibi
Ulaş PİRİM

 [Gereksininim Analizi](https://github.com/ulaspirim/tenis_kulubu/blob/main/Gereksinim_Analizi.md)
 
# Uygulama Yapım Aşamaları ve Test Süreci

## 6-) Front-End Tasarımı
Uygulamada flutter yani dart yazılım dilini kullanarak arayüzü oluşturdum. Arka planda Google'nin firebase altyapısını kullanarak veritabanı gibi çalışmaları sergiledim.

 [Flutter Yazılım Dosyası](https://github.com/ulaspirim/tenis_kulubu/tree/main)
 
## 7-) Kontrol ve Test
 [Mobil Uygulama Testleri](https://youtube.com/shorts/ivkp-HrG9Cw?feature=share)
